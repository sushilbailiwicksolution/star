...
const replaceMissingEventNames = (data) => {
  return data.map(row => ({
    ...row,
    event_name: row.event_name || 'NA'
  }));
};


  const sendData= {"data":Object.assign({},flightFuelData), "flightData":flightData?flightData:flightQueryData , "eventCount":totalCounrData}
  res.status(200).end(JSON.stringify(Object.assign({},sendData)))
  ...
  // console.log(" -- Test2 --");
          ...
          //  console.log(" -- Test3 --["+Date(Date.now()).toString()+"] " + report_data_third_str);
           ...
           try {
             ...
             const flightIdValue = locationQueryResult.rows[0].flight_id_value;
