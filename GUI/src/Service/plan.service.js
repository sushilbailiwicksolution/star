...
const apiUrl_eventList =
  ...
  const apiUrl_reportNotification= process.env.REACT_APP_URL +
  process.env.REACT_APP_URL_OTHER_PORT_NEST_APIS + APP_SERVICE_V +REPORT_NOTIFICATION;
...
const createReportNotification =(requestParams)=>{
  return new Promise((resolve, reject) => {
    axios
      .post(apiUrl_reportNotification, JSON.stringify(requestParams), {
        headers,
      })
      .then((results) => {
        const resultData = results && results.data ? results.data : false;
...
const getReportNotification=(customerId,reportName)=>{
  return new Promise((resolve, reject) => {
    axios
      .get(`${apiUrl_reportNotification}/${customerId}/${reportName}`)
      .then((results) => {
        const resultData = results && results.data ? results.data : false;
        if (resultData) {
          let obj = { status: 200, data: resultData };
...
const updateReportNotification=(data , reportName , customerId)=>{
  return new Promise((resolve, reject) => {
    axios
      .put(
        `${apiUrl_reportNotification}/${customerId}/${reportName}`,
        JSON.stringify(data),
        {
          headers,
        }
