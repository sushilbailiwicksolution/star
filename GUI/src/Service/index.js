...
import {
  ...
  PORT_9001,
  GET_FLIGHT_DETAILS,
  GET_FLIGHTS_FOR_ASSET,
  GOOGLE_API_KEY,
  APP_SERVICE,
  GET_LatestFlightsForAssetDateOrLimit,
  GET_FLIGHT_DETAILS_FORGRAPH,
...
export const getLatestFlightsForAssetDateOrLimit = (payload) => {
  const apiUrl = `${API_URL_FLIGHT}${GET_LatestFlightsForAssetDateOrLimit}`;
  return new Promise((resolve, reject) => {
    axios
...
export const getAirCraftDetailsForGraoh = (airCraftId ,XAXIS , YAXIS) => {
  //let apiUrl = 'http://103.10.234.248:8081/getFlightDetails';
  //let apiUrl = 'http://103.10.234.158:8081/getFlightDetails';
  let apiUrl = `${process.env.REACT_APP_URL}${process.env.REACT_APP_URL_OTHER_PORT_FLIGHT}${GET_FLIGHT_DETAILS_FORGRAPH}`;
  return new Promise((resolve, reject) => {
    if (airCraftId) {
      ...
      axios.get(`${apiUrl}/${airCraftId}`, {params}).then((results) => {
        const resultData = results && results.data ? results.data : false;
        if (resultData) {
          return resolve(resultData);
        } else {
