...
//Get user list based on customer id  

export const getUsersListForCustomer = (userId) => {
  return new Promise((resolve, reject) => {
    axios
      .get(`${apiUrl}/customer/${userId}`)
      .then((results) => {
        const resultData = results && results.data ? results.data : false;
        if (resultData) {
          let obj = {
...
/** Update User */
export const updateUser = (UserId,requestParams) => {
  return new Promise((resolve, reject) => {
    axios
      .put(`${apiUrl}/${UserId}`, JSON.stringify(requestParams), {
        headers
      })
      .then((results) => {
        const resultData = results && results.data ? results.data : false;
