...
import { LineChart, Label, XAxis, YAxis, Tooltip, Legend, Line } from 'recharts';
import { viewService } from '../../Service/view.service';
import { getAirCraftDetails } from '../../Service';
...
const LargeWindow = (props: any) => {
    ...
    useEffect(() => { setAirCraftJson(data) }, [data])
    ...
    const lineChart = () => {
        ...
        if (xAxisKey != 'date_time') {
            xMin = airCraftjson.reduce((min, b) => Math.min(min, b[xAxisKey]), data[0][xAxisKey]);
            xMax = airCraftjson.reduce((max, b) => Math.max(max, b[xAxisKey]), data[0][xAxisKey]);
        }

        let yMin = "";
        let yMax = "";
        if (yAxisKey != 'date_time') {
            yMin = airCraftjson.reduce((min, b) => Math.min(min, b[yAxisKey]), data[0][yAxisKey]);
            yMax = airCraftjson.reduce((max, b) => Math.max(max, b[yAxisKey]), data[0][yAxisKey]);
        }

        return <LineChart width={730} height={450} data={airCraftjson}
            ...
            {yAxisKey === 'date_time' ? <YAxis dataKey={yAxisKey} label={{ value: yAxisLabel, angle: -90, position: 'insideLeft' }}>
            </YAxis> : <YAxis type="number" domain={[yMin, yMax]} label={{ value: yAxisLabel, angle: -90, position: 'insideLeft' }}>
            </YAxis>}
                                        ...
                                        <p className='m-0 cl-white text-left event-style'>
                                            <div className='select-dropdown'>
                                                <select
                                                    ...
                                                    value={selected_X_Axis.label ? selected_X_Axis.label : ''}
                                                >
                                                    <option value={''}>Select X-axis</option>
                                                    {graphDropdown.map((item: any) => {
                                                        return <option key={item.key} value={item.label} >
                                                            {item.label}
                                                        </option>
                                                    })}
                                        ...
                                        <p className='m-0 cl-white text-left event-style'>
                                            <div className='select-dropdown'>
                                                <select
                                                    ...
                                                    value={selected_Y_Axis.label ? selected_Y_Axis.label : ''}
                                                >
                                            ...
                                            <div className='select-dropdown'>
                                                ...
                                                >
                                                    ...
                                                    {flightsList.map((item: any) => {
                                                        return (
                                                            <option value={item.aircraftid} key={item.aircraftid}>
                                                                {item.aircraftid}
                                                            </option>
                                                        );
                                                    })}
