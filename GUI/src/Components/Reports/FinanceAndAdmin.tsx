...
import { GET_REPORT, GET_REPORT_FINANCE_ADMIN } from "../../Config/siteConfig";
import { fixDecimalService } from "../../Service/fixDecimal.service";

const FinanceAndAdmin = (props: any) => {
  ...
  const handlePageClick = (e: any) => {
    ...
    for (let i = start; i <= end; i++) {
      //newData.push(JSONDATA[i]);
      newData.push(reportResponse.financeAndAdminData.report_data_s2[i]);
    }
    updateReportsList(newData);
  };

  useEffect(() => {
    let data = reportResponse.financeAndAdminData;
  ...
  }, [reportResponse.financeAndAdminData]);
  ...
  const downloadExcel = () => {
    const fileName = "FinanceAndAdmin";
    const exportType = "xls";
    exportFromJSON({
      data: reportResponse.financeAndAdminData.report_data_s2,
      fileName,
      exportType,
    });
  };
  ...
  const downloadPdf = () => {
    const reportName = "FINANCE AND ADMINISTRATION";
    const url = `${process.env.REACT_APP_URL}${process.env.REACT_APP_URL_OTHER_PORT_REPORT}${GET_REPORT}${GET_REPORT_FINANCE_ADMIN}/${reportResponse.selectedFlight}/${reportName}/pdf`;
        ...
        <div className="col-md-12">
          <div className="row">
            <div className="col-md-8 d-flex align-items-center justify-content-between">
              <h2 className="cl-white mr-5">Finance And Admin</h2>
              {showData === true ? (
...
export default memo(FinanceAndAdmin);
