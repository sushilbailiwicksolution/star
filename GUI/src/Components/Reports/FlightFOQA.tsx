...
import { GET_REPORT, GET_REPORT_FLIGHT_FOQA } from "../../Config/siteConfig";
...
const FlightFOQA = (props: any) => {
  ...
  const downloadExcel = () => {
    ...
    exportFromJSON({
      data: reportResponse.foqaData.report_data_s2_moqa_1,
      fileName,
      exportType,
    });
  };
  ...
  const downloadPdf = () => {
    const reportName = "FOQA";
    const url = `${process.env.REACT_APP_URL}${process.env.REACT_APP_URL_OTHER_PORT_REPORT}${GET_REPORT}${GET_REPORT_FLIGHT_FOQA}/${reportResponse.selectedFlight}/${reportName}/pdf`;
