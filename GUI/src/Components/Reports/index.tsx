...
import OOOIReports from './OOOIReports';
...
function Reports(props: any) {
  ...
  const getReports = (flightId: any) => {
    ...
    if (activeReport == reports.OOOI) {
      dispatch(reportActions.getReportFlightOOOI(flightId));
    }
  };

  // useEffect(() => {
  //   const currentPath = history.location.pathname;
  //   const reportName = currentPath.split('/').pop();
  //   setActiveReport(reportName || defaultActiveReport);
      ...
      <div className='container-fluid content-body vh-100'>
        <div className='row h-100vh-80px'>
          ...
          <Switch>
            ...
            <Route exact path={`${path}/${reports.OOOI}`}>
              <OOOIReports />
            </Route>
          </Switch>
        </div>
