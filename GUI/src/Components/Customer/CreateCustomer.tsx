...
function CreateCustomer(props: any) {
  ...
  // const [preLogoText, setPreLogoText] = useState("");
  // const [postLogoText, setPostLogoText] = useState("");
  ...
  const updateValues = (data: any) => {
    let obj = {
      ...
      preLogoText:data.preLogoText,
      postLogoText:data.postLogoText,
      ...
      user: {
        ...
        neverExpire: data?.user?.neverExpire,
        expires_on: data?.user?.expires_on,
      },
    };
    setFormFields({ ...obj })
  ...
  const updateHandleSubmit = (e: any) => {
    ...
    if (isValid) {
      const requestParams: any = formFields;
      requestParams.logo_filename=logoFileName;
      requestParams.user.accountType = "CUSTOMER";
      requestParams.user.userType = "customer";
      requestParams.user.customerId = null;
      requestParams.user.username = requestParams.name;
  ...
  const validateForm = () => {
    ...
    const { name, email, address, city, state, country, isAdmin, user, preLogoText,postLogoText } =
      formFields;
    ...
    if (preLogoText) {
      formError.preLogoText = "";
    } else {
      returnValue = false;
      formError.preLogoText = "Please enter Pre Logo Text";
    }
    if (postLogoText) {
      formError.postLogoText = "";
    } else {
      returnValue = false;
      formError.postLogoText = "Please enter Post Logo Text";
    }
  ...
  const createCustomer = async (request: any) => {
    ...
    try {
      const requestData = {
        ...request,
        logo_filename: logoFile ? `${request.name}_${logoFile.name}` : null,
      };
      // console.log('Customer data', requestData);
      
      let data = await customerService.createCustomer(requestData);
      
      if (logoFile) {
        await uploadLogo(request.name);
      }
      setLoader(false);
      if (data.status == 200) {
  ...
  const uploadLogo = async (customerName: string) => {
    if (!logoFile) {
    ...
    try {
      // Use your logo upload API endpoint
      let response = await fetch(`https://star-ads-app.com:3378/customer/uploadLogo?name=${customerName}`, {
        method: "POST",
        body: formData,
      });
  ...
  const updateCustomer = async (request: any) => {
    try {
      const requestData = {
        ...request,
        logo_filename: logoFile ? `${request.name}_${logoFile.name}` :request.logo_filename,
      };
      let data = await customerService.updateCustomer(requestData, userId);
      if (logoFile) {
        await uploadLogo(request.name);
      }
      setLoader(false);
  ...
  <div className="col-md-4">
    <div className="form-group mb-3">
      <label className="d-block text-left cl-white">Pre Logo Text</label>
      <input
        type="text"
        className="form-control"
        name="preLogoText"
        placeholder="Pre Logo text..."
        onChange={handleChange}
        value={formFields.preLogoText}
        disabled={isReadonly}
      />
    </div>
  </div>
  <div className="col-md-4">
    <div className="form-group mb-3">
      <label className="d-block text-left cl-white">Post Logo Text</label>
      <input
        type="text"
        className="form-control"
        name="postLogoText"
        placeholder="Post Logo Text..."
        onChange={handleChange}
        value={formFields.postLogoText}
        disabled={isReadonly}
      />
    </div>
  </div>
...
<div className="row mb-3">
  <div className="col-md-4">
    <div className="form-group mb-3">
      <label className="d-block text-left  cl-white">Customer Logo</label>
