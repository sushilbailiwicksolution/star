...
const LiveTrackingMap = forwardRef((props: any, ref) => {
  ...
  const[selectedAircraftFlightId , changeSelectedAircraftFlightId] = useState<any>("");
  ...
  const Assets = () => {
    return (
      <div className="color-white" onClick={() => setToggleContextMenu(false)}>
        <div className="row">
              ...
              >
                <div className="list-row">
                  ...
                  <div className="list-child-2">
                    ...
                    </p>{item.current_status === "IN-AIR" ? (
  <p style={{fontSize: '10px' }}>Age: &lt; 1 min ago</p>
) : (
  <>
    {item.elapsed_time.days === 0 && item.elapsed_time.hours === 0 ? (
      <p style={{ display: 'inline', fontSize: '10px' }}>Age: {item.elapsed_time.minutes} mins ago</p>
    ) : null}
  </>
)}
...
<div className="list-child-3">
  <p>{item.current_status}</p>
  <p className="text-truncate">{item.location}</p>
  {item.current_status === "ON-GROUND" && (
  <p>
    {item.elapsed_time.days > 0 && `${item.elapsed_time.days} days `}
    {item.elapsed_time.days > 0 || item.elapsed_time.hours > 0 ? `${item.elapsed_time.hours} hrs ` : ''}
    {item.elapsed_time.days > 0 || item.elapsed_time.hours > 0 || item.elapsed_time.minutes > 0 ? `${item.elapsed_time.minutes} mins ` : ''}
    ago
  </p>
)}
  ...
  return (
    ...
    >
      ...
      <div className="sidebar-wrapper noheight-wrapper noheight-wrapper-view p-4">
        ...
        {selectedTab === 2 ? (
          <Events selectedAicraftId={selectedAicraftId}  selectedAircraftFlightId={selectedAircraftFlightId}/>
        ) : null}
