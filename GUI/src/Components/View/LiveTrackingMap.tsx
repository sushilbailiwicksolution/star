...
const LiveTrackingMap = forwardRef((props: any, ref) => {
  ...
  const selectedAssetIndex = useSelector((state: any) => state.viewReducer.selectedAssetIndex);
  ...
  useEffect(() => {
    ...
    } else if (filterData.filter && !sortData.sort) {
  ...
  const onClickMoveToAsset= (e:any)=>{
    e.preventDefault();
   //get The itemId for which the event took place 
  //  console.log('Plane Data', selectedAssetData);
   
   if(selectedAssetData){
    const moveObj = {
      lat: selectedAssetData.gps_lat,
      long: selectedAssetData.gps_long,
    };
  ...
  const onClickZoomToAsset=(e:any)=>{
    e.preventDefault();
    if(selectedAssetData){
      const zoom = {
        lat: selectedAssetData.gps_lat,
  ...
  const onClickMoveToAssetEvents= (e:any)=>{
    e.preventDefault();
    setSelectedTab(2);
  }
  ...
  //     if (
  //       selectedAssetItem &&
  //       selectedAssetItem.current_status != "OFF"
  //     ) {
  //       getAirCraftDetailsAPI(selectedFlight);
  //     }
  //     if (selectedAssetItem?.current_status == "OFF" && count == 0) {
  //       count = 1;
  //       getAirCraftDetailsAPI(selectedFlight);
  //     }
  //     console.log('Selected flight each interval',  selectedFlight);
  ...
  const getLocationValue = (assetItem: any) => {
    if (assetItem.current_status === "OFF") {
      return assetItem.location;
    } else {
      return `LOC: ${assetItem.final_gps_lat},${assetItem.final_gps_long}`;
    }
  };

  useEffect(() => {
    updateSelectedAsset();
  }, [selectedAssetIndex, assetsList]);

  const updateSelectedAsset = () => {
    if (selectedAssetIndex >= 0 && selectedAssetIndex < assetsList.length) {
      // console.log('Index asset from arrow' , selectedAssetIndex);
      const selectedAssetData:any = assetsList[selectedAssetIndex];
      setSelectedAsset(selectedAssetData.id);
      changeSelectedAircraftId(selectedAssetData.name);
      changeSelectedAircraftFlightId(selectedAssetData.aircraftid);
      setSelectedAssetData(selectedAssetData);
      // console.log('Changing asset from arrow' , selectedAssetData);
              ...
              <div className="col-md-3 d-flex text-align-center align-item-center">
                <>
                  ...
                  <Menu
                    ...
                    title="Filter assets based on current status"
                  >
                ...
                <>
                  ...
                  <Menu
                    ...
                    title="Arrange assets in prefer order"
                  >
        ...
        <div className="list-group asset-list">
          {assetsList.map((item: any) => {
            return (
              ...
              >
                <div className="list-row" title="Latest flight Data">
                  <img
      ...
      item.elapsed_time.minutes < 5? (
        ...
        item.elapsed_time.minutes > 5? (
          <p>Age: {item.elapsed_time.minutes} mins ago</p>
                  ...
                  <div className="list-child-3">
                    <p>{item.current_status}</p>
                    <p className="text-truncate">{item.location?item.location:`LOC: ${item.final_gps_lat},${item.final_gps_long}`}</p>
                    
                      

{/* <p>
  Age:{" "}
  {item.elapsed_time.days > 0 && `${item.elapsed_time.days} days `}
  {(item.elapsed_time.days > 0 ||
    item.elapsed_time.hours > 0) &&
    `${item.elapsed_time.hours} hrs `}
  {(item.elapsed_time.days > 0 ||
    item.elapsed_time.hours > 0 ||
    (item.elapsed_time.minutes > 3 &&
      item.elapsed_time.minutes > 0)) &&
    `${item.elapsed_time.minutes} mins `}
  {(item.elapsed_time.days ===0 &&
    item.elapsed_time.hours === 0 &&
    item.elapsed_time.minutes >0) &&
    (item.elapsed_time.minutes <= 3 ? "< 1 min " : "")}
  ago
</p> */}
<p>Age: {item.elapsed_time}</p>
        ...
        <div className="custom-card">
          ...
          >
            <div className="custom-card-header">
              <ul className="nav">
                <li className="nav-item" title="Move To Asset">
                  <a href="#" className="nav-link text-white px-2"  onClick={onClickMoveToAsset}>
                    <OpenInNewIcon fontSize="large"  />
                  </a>
                </li>
                <li className="nav-item" title="Zoom To Asset">
                  <a href="#" className="nav-link text-white px-2" onClick={onClickZoomToAsset}>
                ...
                <li className="nav-item" title="Asset Events">
                  <a href="#" className="nav-link text-white px-2" onClick={onClickMoveToAssetEvents}>
            ...
            <div className="custom-card-body">
              <table className="table table-borderless">
                ...
                <tbody>
                  {selectedAssetData?.esn ? (
                    <>
                      <tr>
                        <td>Name</td>
                        <td>{selectedAssetData.vehicletype}</td>
                      </tr>
                      <tr>
                        <td>ESN</td>
                      ...
                      <tr>
                        <td>Lat/Long</td>
                        <td>{`${selectedAssetData.final_gps_lat?selectedAssetData.final_gps_lat:'-'},${selectedAssetData.final_gps_long?selectedAssetData.final_gps_long:'-'}`}</td>
                      </tr>
                      <tr>
                        <td>Speed</td>
                        <td>{selectedAssetData.speed?upToTwoDecimal(selectedAssetData.speed):0} knots</td>
                      </tr>
                      <tr>
                        <td>Heading</td>
                      ...
                      <tr>
                        <td>Altitude</td>
                        <td>{selectedAssetData.altitude?upToTwoDecimal(selectedAssetData.altitude):0} ft</td>
                      </tr>
                      <tr>
                        <td>Distance</td>
                        <td>{selectedAssetData.distance} meters</td>
                      </tr>
                      <tr>
                        <td>IMEI</td>
                      ...
                      <tr>
                        <td>Asset Make</td>
                        <td>{selectedAssetData.assetMake}</td>
                      </tr>
                      <tr>
                        <td>Description</td>
                      ...
                      <tr>
                        <td>Asset Model</td>
                        <td>{selectedAssetData.assetModel}</td>
                      </tr>
                    </>
                  ) : <>
                  <div style={{ backgroundColor: 'yellow', textAlign: 'center', color: 'black', borderRadius: '20px', marginLeft:'50px', height: '100px' }}>
                    <p>No asset selected</p>
                    <span>Select an asset above to view information</span>
                  </div>
                </>
      ...
      <div className="sidebar-wrapper noheight-wrapper noheight-wrapper-view p-4">
        ...
        <div className="view-tabs">
          <span
            ...
            title="Contains most recent flight"
          >
            Assets
          </span>
          <span
            ...
            title="Contains Alarming Events"
          >
            Events
          </span>
          <span
