...
const DataLayers = () => {
    ...
    const getDataLayerData = async () => {
        try {
            ...
            setLoader(false);
            dataLayerData.results ? setLayerData(dataLayerData.results) : setLayerData([]);
        } catch (error) {
            setLayerData([]);
            setLoader(false);
        }
    ...
    const handleOnChange = (has_subcategory: boolean, parentIndex: number, childIndex: number, grandChildIndex?: number) => {
        if (has_subcategory) {
            const layer_data: any = [...layerData];
            if (grandChildIndex != undefined) {
                const item = layer_data[parentIndex].data[childIndex].data[grandChildIndex];
        ...
        <>
            ...
            <div className="color-white">
                <div>
                    ...
                    >
                        {layerData.map((item: any, parentIndex: any) => {
                            return <TreeItem nodeId={item.id} label={item.category} sx={{ textAlign: 'left' }}>
                                {item.data.map((data: any, childIndex: any) => {
                                    if (item.has_subcategory) {
                                        return <TreeItem nodeId={`${item.id}_${data.id}`} label={data.value} sx={{ textAlign: 'left' }} >
                                            {data.product.map((data2: any, grandChildIndex: number) => {
                                                ...
                                                const label = (
                                                    <div style={{ display: 'flex', alignItems: 'center' }}>
                                                        <Checkbox
                                                            ...
                                                            onChange={() => handleOnChange(item.has_subcategory, parentIndex, childIndex, grandChildIndex)}
                                                        />
                                                        <Typography variant="body2" sx={{ fontSize: '1rem' }}>{data2.product_name}</Typography>
                                                    </div>
                                                );
                                                return <TreeItem nodeId={`${item.id}_${data.id}_${grandChildIndex}_${data2.product_id}`} label={label} sx={{ textAlign: 'left' }} />
                                            })}
                                        </TreeItem>
                                    } else {
                                        ...
                                        const label = (
                                            <div style={{ display: 'flex', alignItems: 'center' }}>
                                                <Checkbox
                                                    ...
                                                    color="primary"
                                                    checked={isChecked ? true : false}
                                                    onChange={() => handleOnChange(item.has_subcategory, parentIndex, childIndex)}
                                                />
                                                <Typography variant="body2" sx={{ fontSize: '1rem' }}>{data.value}</Typography>
                                            </div>
                                        );
                                        return <TreeItem nodeId={`${item.id}_${data.id}`} label={label} sx={{
                                            textAlign: 'left'
                                        }} >
                                        </TreeItem>
                                    }
                ...
                <div className="custom-card">
                    <button className="btn btn-primary px-3 py-2 mb-3" onClick={() => setDataTogglebtn(!dataTogglebtn)}><i className={dataTogglebtn ? 'fas la-angle-down' : 'fas la-angle-up'}></i></button>
                    <div className={dataTogglebtn ? 'custom-card-container show-top' : 'custom-card-container'}>
                        <div className="custom-card-header">
                            <ul className="nav justify-content-end">
                                <li className="nav-item"><a href="#" className="nav-link text-white"><i className="fas la-trash-alt"></i></a></li>
                                <li className="nav-item"><a href="#" className="nav-link text-white"><i className="fas la-home"></i></a></li>
                                <li className="nav-item"><a href="#" className="nav-link text-white"><i className="fas la-pen"></i></a></li>
                                <li className="nav-item"><a href="#" className="nav-link text-white"><i className="fas la-external-link-alt"></i></a></li>
                                <li className="nav-item"><a href="#" className="nav-link text-white"><i className="fas la-save"></i></a></li>
                            </ul>
                        </div>
