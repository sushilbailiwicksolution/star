...
function MapContainer(props) {
  ...
  useEffect(() => {
    googleMap.current.map.setOptions({
      mapTypeId: viewReducerResponse.mapTypeId,
    });
  }, [viewReducerResponse.mapTypeId]);
  const zoomControlOptions = {
    position: props.google.maps.ControlPosition.TOP_LEFT,
  };

  useEffect(() => {
    if (viewReducerResponse.zoomInTrigger > 0) {
      const selectedZoomLevel = googleMap.current.map.zoom;
      if (selectedZoomLevel < MaxZoomLevel) {
        googleMap.current.map.setOptions({
          zoom: selectedZoomLevel + 1,
        });
      }
    }
  }, [viewReducerResponse.zoomInTrigger]);

  useEffect(() => {
    if (viewReducerResponse.zoomOutTrigger > 0) {
      const selectedZoomLevel = googleMap.current.map.zoom;
      if (selectedZoomLevel > MinZoomLevel) {
        googleMap.current.map.setOptions({
          zoom: selectedZoomLevel - 1,
        });
      }
    }
  }, [viewReducerResponse.zoomOutTrigger]);

  useEffect(() => {
    const defaultZoomLevel = 3;
    const defaultInitialCenter = { lat: 23.3, lng: 22.2 };
    googleMap.current.map.setOptions({
      zoom: defaultZoomLevel,
      initialCenter: defaultInitialCenter,
      center: defaultInitialCenter,
    });
  }, []);

  useEffect(() => {
    if (viewReducerResponse.moveToAsset) {
      const initialCenter = {
        lat: Number(viewReducerResponse.moveToAsset.lat),
        lng: Number(viewReducerResponse.moveToAsset.long),
      };
      googleMap.current.map.setOptions({
        center: initialCenter,
      });
    }
  }, [viewReducerResponse.moveToAsset]);

  useEffect(() => {
    if (viewReducerResponse.zoomToAsset) {
      const mapStateData = { ...mapState };
      mapStateData.initialCenter = {
        lat: viewReducerResponse.zoomToAsset.lat,
        lng: viewReducerResponse.zoomToAsset.long,
      };
      const zoomLevel = 15;
      mapStateData.zoom = zoomLevel;
      googleMap.current.map.setOptions({
        zoom: zoomLevel,
      });
      setMapState(mapStateData);
    }
  }, [viewReducerResponse.zoomToAsset]);
    ...
    if (viewReducerResponse.currentFlightList.length > 0) {
      ...
      const displayMarkers = allLastLngs.map((store, index) => {
        ...
        if (Latitude && Longitude) {
          ...
          let icon = getIconValue(event_name, 1.5, store.heading);
      ...
      const latstFlightId =
        mapState.displayMarkers.length > 0
          ? mapState.displayMarkers[0].props.data.aircraftid
          : null;

      if (!latstFlightId || latstFlightId != allLastLngs[0].aircraftid) {
        googleMap.current.map.setOptions({
          zoom: zoomLevel,
        });
      } else {
        initialCenter = mapState.displayMarkers[0].props.data.initialCenter;
      }
  ...
  useEffect(() => {
    if (viewReducerResponse.assetList.length > 0) {
      const assetMarkers = viewReducerResponse.assetList.map((item) => {
        let position = {
          lat: item.gps_lat,
          lng: item.gps_long,
        };
        const icon = getIcon(1.5, item.heading);
        return getMarker(item.name, icon, position, item);
      });
    ...
    function getMarker(uniqueKey, icon, position, data) {
      const shape = {
    ...
    function getIcon(scale, rotation) {
      return {
        // url: planeIcon,
        path: "M26.9719 10.2611C28.2048 9.88148 28.6889 9.91391 29.7116 8.89121C30.4455 8.15727 30.0888 7.48775 29.5062 6.90509C28.3194 5.71835 25.3818 5.43179 23.8212 5.7405C23.5482 5.30178 20.9544 1.50386 20.5336 1.08304C20.2376 0.787089 16.1191 0.834189 15.602 0.535156C15.5545 0.8427 16.0753 2.14418 16.2202 2.52142L18.4314 8.48037C19.1408 10.5039 20.3921 13.5113 21.0364 15.4667C21.1617 15.847 21.3174 16.1483 21.4239 16.494C21.1856 16.5412 20.7629 16.5631 20.465 16.5886L15.6705 17.0429C15.3615 17.0813 15.054 17.1071 14.7116 17.1366C13.7606 17.2185 6.38861 17.8233 6.08136 18.0009L2.51967 12.7955L0.327984 12.7622L0.293945 28.2749L2.51982 28.3081C2.78393 27.9627 3.18846 27.324 3.42293 26.9736L6.08136 23.0695C6.33916 23.2187 6.73416 23.1937 7.04023 23.2257L9.917 23.4995C10.5679 23.5682 11.168 23.6037 11.8349 23.6596C12.5059 23.7161 13.083 23.8172 13.7526 23.8559L21.4239 24.5764L15.8867 39.5078C15.7272 39.89 15.5479 40.1849 15.602 40.5352C15.8415 40.3966 16.5104 40.3718 16.835 40.3639C17.1333 40.3567 17.1741 40.3394 17.4512 40.3096C21.2612 39.8997 19.9152 40.865 22.2164 37.6585C22.77 36.8872 23.351 36.0855 23.8212 35.3298C27.4169 36.0411 31.4359 33.9035 29.7116 32.1791C28.689 31.1566 28.2048 31.1888 26.9719 30.8092C27.2537 30.5145 27.5651 30.0138 27.8249 29.6448L30.3591 26.0146C30.5049 25.8077 30.9933 25.0224 31.1682 24.859C31.5034 24.5454 35.2598 24.2609 35.9445 24.1843L39.5745 23.7459C40.4069 23.6281 41.1773 23.5509 41.9719 23.339C43.3369 22.9753 43.897 22.6511 44.8486 21.6996C45.3507 21.1975 45.3507 19.8728 44.8487 19.3709C43.2118 17.734 41.762 17.634 39.5747 17.3246C39.1512 17.2647 38.8162 17.2434 38.4102 17.1978L36.013 16.8944C35.563 16.8388 35.2306 16.8234 34.78 16.7595C30.7413 16.188 31.6399 16.8753 30.3591 15.0557L27.8729 11.494C27.6082 11.1122 27.257 10.5594 26.9719 10.2611Z",
        fillColor: "#FF2C5C",
  ...
  }, [viewReducerResponse.assetList]);

  useEffect(() => {
    const selectedEvent = viewReducerResponse.selectedEvent;
    console.log("selectedEvent", selectedEvent);

    if (selectedEvent) {
      const lat = selectedEvent.gps_lat;
      const lng = selectedEvent.gps_long;

      if (mapState.displayMarkers) {
        const findMarker = mapState.displayMarkers.find(
          (item) =>
            item &&
            item.props.position.lat == lat &&
            item.props.position.lng == lng
        );

        console.log("selected event present in waypoint", findMarker.props);

        if (findMarker) {
          handleMarkerClick(findMarker.props);
          const initialCenter = {
            lat: Number(lat),
            lng: Number(lng),
          };
          console.log("initialCenter onclick event", initialCenter);
          googleMap.current.map.setOptions({
            initialCenter: initialCenter,
            center: initialCenter,
          });
  ...
  }, [viewReducerResponse.selectedEvent]);
  ...
  const handleMarkerClick = (props) => {
    ...
    let alreadySelectedMarker = mapState.selectedMarker;
    ...
    let data = props.data;
    let markerIndex = props.markerIndex;
    let scaleValue = 3;
    ...
    if (alreadySelectedMarker) {
      ...
      if (alreadySelectedMarker.id == routeId) {
        ...
        let previousSelectedIcon = getIconValue(
          previousEventName,
          1.5,
          previousRotation
        );

        previousSelectedMarker = _getMarker(
    ...
    let updatedIcon = getIconValue(event_name, scaleValue, rotation);
    let updateMarker = _getMarker(
      routeId,
      position,
      markerIndex,
      updatedIcon,
    ...
    let displayMarkers = [...mapState.displayMarkers];

    displayMarkers[props.markerIndex] = updateMarker;
    if (changePreviousSelectedMarker) {
      displayMarkers[previousMarkerIndex] = previousSelectedMarker;
    }
    // 2. Set the state to our new copy
    setMapState({ ...mapState, displayMarkers, selectedMarker: updateMarker });
  };
  ...
  const getIconValue = (eventName, scale, rotation) => {
            ...
            >
              {/* <li>
                ...
                {infoPopup.selectedPlace.data
                  ? `${infoPopup.selectedPlace.data['gps_lat']},${infoPopup.selectedPlace.data['gps_long']}`
                  : ''}
              </li>
              <li>
                Heading:{' '}
              ...
              <li>
                timestamp:{infoPopup.selectedPlace.data ? infoPopup.selectedPlace.data['date_time'] : ''}
              </li>
              <li>
                altitude:{infoPopup.selectedPlace.data ? infoPopup.selectedPlace.data['altitude'] : ''}
              </li>
              <li>
                speed:{infoPopup.selectedPlace.data ? infoPopup.selectedPlace.data['speed'] : ''}
              </li> */}

              {infoPopup.selectedPlace.data &&
              infoPopup.selectedPlace.data["asset_id"] ? (
                <li>
                  Asset:{" "}
                  {infoPopup.selectedPlace.data
                    ? `${infoPopup.selectedPlace.data["asset_id"]}`
              ...
              {infoPopup.selectedPlace.data &&
              infoPopup.selectedPlace.data["event_name"] ? (
                <li>
                  Event Name:{" "}
                  {infoPopup.selectedPlace.data
                    ? `${infoPopup.selectedPlace.data["event_name"]}`
              ...
              <li>
                Timestamp:{" "}
                {infoPopup.selectedPlace.data
                  ? convertTime(infoPopup.selectedPlace.data["date_time"])
                  : ""}
              </li>
              <li>
                Location:
                {infoPopup.selectedPlace.data
                  ? `${infoPopup.selectedPlace.data["gps_lat"]}${
                      infoPopup.selectedPlace.data["gps_lat"] > 0 ? "°N" : "°S"
                    }, ${infoPopup.selectedPlace.data["gps_long"]}${
                      infoPopup.selectedPlace.data["gps_long"] > 0 ? "°E" : "°W"
                    }`
                  : ""}
              </li>
              <li>
                ...
                {infoPopup.selectedPlace.data
                  ? `${upToTwoDecimal(
                      infoPopup.selectedPlace.data["altitude"]
                    )}ft`
                  : ""}
              </li>
              <li>
                ...
                {infoPopup.selectedPlace.data
                  ? `${upToTwoDecimal(
                      infoPopup.selectedPlace.data["speed"]
                    )} knots`
                  : ""}
              </li>
              <li>
              ...
              {infoPopup.selectedPlace.data &&
              infoPopup.selectedPlace.data.packet_type &&
              infoPopup.selectedPlace.data["packet_type"] != "H" ? (
                <>
                  <li>
                    Distance:{" "}
                    {infoPopup.selectedPlace.data &&
                    infoPopup.selectedPlace.data["distance"]
                      ? `${infoPopup.selectedPlace.data["distance"]}`
                      : ""}
                  </li>
                  <li>
                    N1(1,2):{" "}
                    {`${upToTwoDecimal(
                      infoPopup.selectedPlace.data["left_engine_n1"]
                    )},${upToTwoDecimal(
                      infoPopup.selectedPlace.data["left_engine_n1"]
                    )}`}
                  </li>
                  <li>
                    EGT/ITT(1,2):{" "}
                    {`${upToTwoDecimal(
                      infoPopup.selectedPlace.data["left_egt"]
                    )},${upToTwoDecimal(
                      infoPopup.selectedPlace.data["right_egt"]
                    )}`}
                  </li>
                  <li>
                    Oil Temp(1,2):{" "}
                    {`${upToTwoDecimal(
                      infoPopup.selectedPlace.data["left_engine_oil_temp"]
                    )},${upToTwoDecimal(
                      infoPopup.selectedPlace.data["right_engine_oil_temp"]
                    )}`}
                  </li>
                  <li>
                    Oil Press(1,2):{" "}
                    {`${upToTwoDecimal(
                      infoPopup.selectedPlace.data["left_engine_oil_press"]
                    )},${upToTwoDecimal(
                      infoPopup.selectedPlace.data["right_engine_oil_press"]
                    )}`}
                  </li>
                </>
              ) : null}
