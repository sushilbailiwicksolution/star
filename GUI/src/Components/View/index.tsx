...
function View(props: any) {
  ...
  const maxIndex = useSelector((state: any) => state.viewReducer.assetList).length;
  ...
  const [isLatency, setIsLatency] = useState(false);
  ...
  const fetchLatency = async () => {
    ...
    try {
      ...
      await viewService.getGraphDropdown(); // Replace with your actual API call
      const endTime = performance.now();
      const latency = (endTime - startTime).toFixed(3);
      setNetworkLatency(`0:${latency}`);
    } catch (error) {
...
const loggedData=logedInUser.results;
  ...
  useEffect(() => {
    // Fetch latency data when the component mounts
    fetchLatency();
  }, []); 
  ...
  const zoomOutMaxMap = () => {
    dispatch({
      type: ViewsConstants.VIEW_ZOOM_OUT_MAX_TRIGGER,
    });
  };

  const onClickPrevArrowIcon = () => { 
    if (selectedAssetIndex > 0) {
      const newIndex = selectedAssetIndex - 1;
      setSelectedAssetIndex(newIndex);
      dispatch({ type: ViewsConstants.SELECTED_ASSET_INDEX, value: newIndex });
    } else {
      // If at the first asset, cycle to the last one
  ...
  const onClickNextArrowIcon = () => {
    if (selectedAssetIndex < maxIndex) {
      const newIndex = selectedAssetIndex + 1;
      setSelectedAssetIndex(newIndex);
      dispatch({ type: ViewsConstants.SELECTED_ASSET_INDEX, value: newIndex });
    } else {
      // If at the last asset, cycle to the first one
      const newIndex = 0;
      setSelectedAssetIndex(newIndex);
      dispatch({ type: ViewsConstants.SELECTED_ASSET_INDEX, value: newIndex });
    }
  ...
  // Event handler for view selection change
  const onChangeView = async(e:any) => {
    const selectedOption = e.target.value;

    // Map selected view to the corresponding location
    switch (selectedOption) {
      case 'Toronto High Overview':
        setGoToValue('Toronto');
        break;
      case 'Europe Icing':
        setGoToValue('Europe');
        break;
      case 'Global Airmets and Sigmets':
        setGoToValue('Saudi');
        break;
      case 'North America Icing':
        setGoToValue('INDIANA');
        break;
      case 'Saudi Arabia Med and High Significant':
        setGoToValue('Riyadh');
        break;
      default:
        setGoToValue('');
          ...
          <div className="col-md-9 my-4 map-box-padding">
            <div className="map-header">
              <ul className="nav align-items-center">
                ...
                <li className="nav-item" onClick={zoomInMap} title="Click to Zoom In">
                  <a href="#" className="nav-link text-white px-2 py-0">
                    <ZoomInIcon fontSize="large" />
                  </a>
                </li>
                <li className="nav-item" onClick={zoomOutMap} title="Click to Zoom Out">
                  <a href="#" className="nav-link text-white px-2 py-0">
                    <ZoomOutIcon fontSize="large" />
                  </a>
                </li>
                <li className="nav-item"  onClick={zoomOutMaxMap} title="Click to Max Zoom Out">
                  <a
                    href="#"
                ...
                <li className="nav-item" onClick={onClickPrevArrowIcon} title="Click to navigate Previous Asset">
                  <a href="#" className="nav-link text-white px-2 py-0">
                    <ArrowBackIcon fontSize="large" />
                  </a>
                </li>
                <li className="nav-item" onClick={onClickNextArrowIcon} title="Click to navigate to Next Asset">
                  <a
                    href="#"
                    className="nav-link text-white px-2 py-0 border-right border-secondary d-flex align-items-center"
                  >
                ...
                <li className="nav-item ml-auto px-2 py-0 border-right border-secondary d-flex align-items-center" title="Enter location name/abbr to navigate on map">
                  <input
                    type="text"
                ...
                <li className="nav-item px-2 py-0 border-right border-secondary d-flex align-items-center">
                  <div className="d-inline-flex align-items-center">
                    ...
                    <select className="form-control map-input" value={selectedView} onChange={onChangeView} >
                      <option value="Toronto High Overview">Toronto High Overview</option>
                      <option value="Europe Icing">Europe Icing</option>
                      <option value="Global Airmets and Sigmets">Global Airmets and Sigmets</option>
                      <option value="North America Icing">North America Icing</option>
                      <option value="Saudi Arabia Med and High Significant">
            ...
            <div className="map-footer">
              <ul className="nav align-items-center">
                <li className="nav-item text-white px-2 py-0 d-inline-flex">
                  ...
                  <span className="pl-2" title="mouse position on map">{onHoverLatLong}</span>
                </li>

                <li className="nav-item ml-auto" title="Alarm">
                  <a href="#" className="nav-link text-white px-2 py-0">
                    <NotificationsIcon
                ...
                <li className="nav-item" title="Messages">
                  <a href="#" className="nav-link text-white px-2 py-0">
                    <EmailIcon fontSize="large"   onClick={onClickAlarm}/>
                  </a>
                </li>
                <li className="nav-item">
                  <a href="#" className="nav-link text-white px-2 py-0">
                ...
                <li className="nav-item text-white px-2 py-0 ml-auto cursor-pointer" onClick={fetchLatency}>
                  {/* <span title="Click to obtain Network Latency">Net. Latency: </span> */}
                  {/* <span className="text-success">{networkLatency}</span> */}
                    <span title="Click to obtain Network Latency">
                  {isLatency ? (
                      <span>
                ...
                <li
                  ...
                  onClick={refreshFlight}
                >
                  <span title="Click to obtain Latest Data">
                    {isUpdating ? (
                      <span>
                        <span className="text-info">Updating...</span>
                      </span>
            ...
            <div className="custom-card position-relative">
              <button
                ...
                title="Click to Obtain Asset and Event Status "
              >
                {mapboxTogglebtn ? (
