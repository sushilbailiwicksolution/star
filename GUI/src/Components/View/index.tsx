...
function View(props: any) {
  ...
  const onInit = async () => {
    try {
      setLoader(true);
      const summaryData = await viewService.getSummaryCount();
      setLoader(false);
      if (summaryData) {
      ...
      >
        ...
        <div className="row">
          ...
          <div className="col-md-9 my-4 map-box-padding">
            ...
            >
              <MapViewContainer ref={mapContainerRef} {...props} />
            </div>
...
<li className="nav-item ml-auto text-white px-2 py-0 cursor-pointer" onClick={refreshFlight}>
  <span>
    {isUpdating ? (
      <span>
      ...
      <span>
        Last update (UTC):{" "}
        <span className="text-success">{viewReducerResponse.lastRefreshedTime}</span>
      </span>
    )}
  </span>
</li>
