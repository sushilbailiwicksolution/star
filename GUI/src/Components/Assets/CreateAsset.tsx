...
import { getAssetsList, planService } from '../../Service/plan.service';
 ...
 /**
  ...
  const updateValues = (data: any) => {
    console.log('update data', data);
     ...
     // Find the corresponding event for Trip Start
  // const startEvent = eventDropdownList.find(
  //   (item: any) => item.eventid === data.start_event_id
  // );

  // // Find the corresponding event for Trip End
  // const endEvent = eventDropdownList.find(
  //   (item: any) => item.eventid === data.end_event_id
  // );

  const startEventValue = data.start_event_id ? data.start_event_id : '';

  // Check if there is an existing value for Trip End
  const endEventValue = data.end_event_id ? data.end_event_id : '';
    ...
    let obj = {
      ...
      enabled: data.status==='ACTIVE' ? true:false,
      description: data.description,
      vehicleOdometer: '',
      start_event_id: startEventValue,
      end_event_id:endEventValue,
    };
  ...
  const checkValidation = () => {
    ...
    if (!assetState.start_event_id.trim()) {
      check = false;
      errors.start_event_id = 'Trip Start is required';
    } else {
      errors.start_event_id = '';
    }

    if (!assetState.end_event_id.trim()) {
      check = false;
      errors.end_event_id = 'Trip End is required';
    } else {
      errors.end_event_id = '';
    }
...
/**
  ...
  const handleSubmit = () => {
    ...
    let requestParams = {
      ...
      owner: customerName,
      start_event_id:assetState.start_event_id,
      end_event_id:assetState.end_event_id,
      vehicletype: assetState.name, 
      countryId: assetState.country,
        ...
        {customers.map((customer) => (
          <option key={customer.id} value={customer.id}>
            {customer.name}
                ...
                <tr>
                <td className='small-shell-2'>
                    <h3 className='edit-heading'>Trip Start</h3>
                  </td>
                  <td>
                    <select
                      className='form-control select-dropdown w-75'
                      ...
                      value={assetState.start_event_id}
                      disabled={isReadonly}
                    >
                      <option value=''>Select Event</option>;
                      {eventDropdownList.map((item: any, index: any) => {
                        return (
                    ...
                    <span className='color-red'>
                      {validation.start_event_id && <p>{validation.start_event_id}</p>}
                    </span>
                  </td>
                  <td className='small-shell-2'>
                    <h3 className='edit-heading'>Trip End</h3>
                  </td>
                  <td>
                    <select
                      className='form-control select-dropdown w-75'
                    ...
                    <span className='color-red'>
                      {validation.end_event_id && <p>{validation.end_event_id}</p>}
                    </span>
                  </td>
                </tr>
              </tbody>
