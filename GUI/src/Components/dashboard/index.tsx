...
function Dashboard(props:any) {
  ...
  const customerId = logedInUser.results.customerId;
      useEffect(() => {
        ...
        if (selectedCustomer) {
            handleCustomerSelection(selectedCustomer.id);
          } 
        else if(accountType==="USER"){
             handleCustomerSelection(customerId)
          }
        else {
            // Set default logo when no customer is selected
            setSelectedCustomerLogo({
              name: 'Welcome Admin',
              logo: require("../../Assets/images/star.jpeg").default,
              PostLogoText:''
            });
          }
      }, [selectedCustomer])
      
    const [selectedCustomerLogo, setSelectedCustomerLogo]= useState({name:'',logo:'', PostLogoText:''});
   ...
   */
    ...
    const handleCustomerSelection = async (customerId:any) => {
       
        try {
          const response = await fetch(`https://star-ads-app.com:3378/customer/getLogo/${customerId}`);
          const customerData = await response.json();
          
          
          setSelectedCustomerLogo({
            name:customerData.preLogoText,
            // logo:`https://star-ads-app.com:3378${customerData.fileUrl}`
            logo: customerData.fileUrl ? `https://star-ads-app.com:3378${customerData.fileUrl}` : require("../../Assets/images/star.jpeg").default,
            PostLogoText:customerData.postLogoText});
        } catch (error) {
         ...
         <div className="container-fluid">
             <div className="row">
                <div className="col-md-12">
                    <p className="Admin-heading">{selectedCustomerLogo.name}</p>
                    {/* <img className="star_dashboard-logo" src={require("../../Assets/images/star.jpeg").default} /> */}
                    <img className="star_dashboard-logo" src={selectedCustomerLogo.logo} alt={selectedCustomerLogo.name} />
                    <p className="Admin-bottom">{selectedCustomerLogo.PostLogoText}</p>
                </div>
