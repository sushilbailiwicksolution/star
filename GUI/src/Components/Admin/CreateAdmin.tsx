...
/**
 * This component is used for creating admin and system Admin.
 * The component consist of handling form field inputs and errors handling for input fields.
 * @component
 */
...
function CreateAdmin(props: any) {
  ...
  /**
   * State variable for the username of the admin.
   * @type {string}
   */
  const [username, setUserName] = useState('');

  /**
   * State variable for form input errors.
   * @type {Object}
   */
  ...
  /**
   * State variable to control the loader.
   * @type {boolean}
   */
  const [loader, setLoader] = useState(true);
/**
   * useEffect to fetch the logged-in user data and set the username state.
   */
  useEffect(() => {
...
/**
  ...
  const addUsers = () => {
    ...
    if (isValid) {
      toast.success(
        `You have added a new admin ${formFields.first_name} ${formFields.last_name}`
      );
      setFormFields({
        login_id: '',
        first_name: '',
      ...
      createAdmin(requestParams);
    }
  };
/**
   * Function to create a new admin user on the server.
   * @param {Object} request - The request object containing user details.
   */
  const createAdmin = async (request: any) => {
    setLoader(true);
    try {
...
/**
   * Function to validate the form fields and update errors state accordingly.
  ...
  /**
   * Event handler function to update the formFields and errors state when input fields change.
