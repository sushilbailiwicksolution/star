...
function sendEmailAlert( emailSubject, emailMessage, emailAddress, assetId, longitude, latitude ){
	...
	var mailOptions = {
		...
		to: 'sushil@bailiwicksolution.com',
		subject: 'Alert',
...
const checkAndSaveGeoDataToPSql = async( data )=>{
}
...
/*function currentDate(){
    var GEO_FENCE_CONF_TABLE = 'geofence_details';
    ...
    var query = "SELECT landmark_lat, landmark_long FROM "+GEO_FENCE_CONF_TABLE+" WHERE asset_id = '"+aircraftId+"' AND "; 

    const client = await pool.connect()
    try{

       const results = await client.query(query)
       //Code to iterate the result and set notification
